...
WEBEX_TEAMS_ROOM_ID = config.WEBEX_TEAMS_ROOM_ID
...
def list_alarms(alarm_tag):
    ...
    query = {
                "query": {
                    ...
                    {
                        "value": [              # Return both active alarms
                        "false","true"
                        ],
                        "field": "active",
                        "type": "string",
...
if __name__ == "__main__":
    ...
    # Use vManage API to list all valid alarm tags
    list_alarms_tags()
    ...
    # Create an incident in ServiceNow using API calls
    print('-'*100)
    title = 'The vEdge router for PodXX has been rebooted!'
    comments = 'There has been a user initiated reboot detected on : PodXX'
    create_incident('PodXX Reboot', comments, snow_user,3)
    # Post alarm details to the class Webex Teams space
    print('-'*100)
    print ('Posting to RoomID '+WEBEX_TEAMS_ROOM_ID)
    print ('using token '+ WEBEX_TEAMS_AUTH)
    message = api.messages.create(WEBEX_TEAMS_ROOM_ID,text=details+' from PodXX')
    print('-'*100)
    print('End of Script')